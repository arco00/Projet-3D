...
public class MobSpawner : MonoBehaviour
{
    ...
    if (timer>=timeSpawn){
            GameObject newCreeper =Instantiate (creeper,Utile.RngPoss(spawn,size,spawn.y),rotation);
            GameObject newGreedy =Instantiate (greedy,Utile.RngPoss(spawn,size,spawn.y),rotation);
            timer=0;
